...
String uploadPath = "C:\\nusangTomcat\\apache-tomcat-9.0.38\\fileupload"; // upload는 폴더명 / 폴더의 경로를 구해옴
//out.print(uploadPath);

try {
	MultipartRequest multi = new MultipartRequest( // MultipartRequest 인스턴스 생성(cos.jar의 라이브러리)
	request, uploadPath, // 파일을 저장할 디렉토리 지정
	1024 * 1024 * 5, // 첨부파일 최대 용량 설정(bite) / 10KB / 용량 초과 시 예외 발생
	"utf-8", // 인코딩 방식 지정
	...
	while (names.hasMoreElements()) {
		...
		orgfileName1 = multi.getOriginalFileName(name);
		out.print(name + "<br>");
		out.print(fileName1 + "<br>");
		out.print(orgfileName1 + "<br>");
		fileName1 = "";
		orgfileName1 = "";
		...
		if(file != null)
		{
			...
			if(bi != null){
				...
				out.println("imageFile : " + width + " x " + height + " y <br>");
			}else{
				out.print("it is not imageFile");
			}
...
<body>
	<%-- <form action="check.jsp" method="post">
		<input name="id" value="<%=id.toString()%>"> <input name="subject" value="<%=subject.toString()%>"> <input name="fileName1" value="<%=fileName1.toString()%>"> <input name="fileName2" value="<%=fileName2.toString()%>"> <input name="orgfileName1" value="<%=orgfileName1.toString()%>"> <input name="orgfileName2" value="<%=orgfileName2.toString()%>"> <input type="submit" value="업로드 확인">
	</form> --%>
</body>
</html>